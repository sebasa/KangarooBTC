  ...
  while( p->isRunning ) {
    ...
    case SERVER_LOADKANG: {
      ...
      while(nbKangaroo > 0) {
        ...
        for(uint32_t k = 0; k < nbK; k++) {
          ::fread(&KBuff[k],sizeof(int256_t),1,f);
          // Checksum
          K.SetInt32(0);
        ...
        PUTFREE("packet",p->clientSock,KBuff,nbK * sizeof(int256_t),ntimeout,KBuff);
    ...
    case SERVER_SAVEKANG: {
      ...
      while(nbKangaroo>0) {
        ...
        GETFREE("packet",p->clientSock,KBuff,nbK * sizeof(int256_t),ntimeout,KBuff);

        for(uint32_t k = 0; k < nbK; k++) {
          ::fwrite(&KBuff[k],sizeof(int256_t),1,f);
          // Checksum
          K.SetInt32(0);
          K.bits64[3] = KBuff[k].i64[3];
          K.bits64[2] = KBuff[k].i64[2];
...
bool Kangaroo::GetKangaroosFromServer(std::string& fileName,std::vector<int256_t>& kangs) {
  ...
  if(!endOfSearch) {
    ...
    while(nbKangaroo > 0) {
      ...
      GETFREE("packet",serverConn,KBuff,nbK * sizeof(int256_t),ntimeout,KBuff);
...
bool Kangaroo::SendKangaroosToServer(std::string& fileName,std::vector<int256_t>& kangs) {
  ...
  if(!endOfSearch) {
    ...
    while(nbKangaroo > 0) {
      ...
      for(uint32_t k = 0; k < nbK; k++) {
        memcpy(&KBuff[k],&kangs[pos],sizeof(int256_t));
        pos++;
        // Checksum
        Int K;
        K.SetInt32(0);
      ...
      PUTFREE("packet",serverConn,KBuff,nbK * sizeof(int256_t),ntimeout,KBuff);
